...
const keyboardFromLoc = localStorage.getItem('lang');
const CapsLK = localStorage.getItem('CapsLK');
...
function init(keyboard, CapsLK) {
  ...
  if (CapsLK === 'true') {
    for (let i = 0; i < keyboard.length; i++) {
      out += `<div class="key" data="${keyboard[i].data}" value="${keyboard[
        i
      ].value.toUpperCase()}"> ${keyboard[i].value.toUpperCase()} </div>`;
    }
  }
  if (CapsLK === 'false') {
    for (let i = 0; i < keyboard.length; i++) {
...
document.onkeydown = function (event) {
  ...
  if (code == 'Win') {
    ...
    (code !== null && event.keyCode !== 16 && code !== 'CapsLK')
    || (code !== 'CAPSLK' && code !== 'Tab')
    || (code !== 'TAB' && code !== 'Alt')
    || (code !== 'ALT' && code !== 'Ctrl')
    || (code !== 'CTRL' && code !== 'Win')
    || code !== 'WIN'
